defmodule ToyRobot do
  ...
  def check(robot,cli_proc_name) do
    pid1 = self()
    pid = spawn_link(fn ->  flag = send_robot_status(robot,cli_proc_name)
    send(pid1,flag)end)
    ...
    cond do
      robot.y == :a and robot.facing== :south -> True
      robot.y == :e and robot.facing== :north -> True
      robot.x == 1 and robot.facing== :west -> True
      robot.x == 5 and robot.facing== :east -> True
      #x -> x
      true -> False
  ...
  def solve(robot,visited,cli_proc_name,goal_x,goal_y) do
    #<<v::utf8>> = robot.y
    v = robot.y
    index = (robot.x-1)*5+ ?v-96
    index_1 = (1 <<< index)

    if (visited &&& index_1) == index_1 do
      False

    end

    visited = visited + index_1

    if(robot.x==goal_x and robot.y==goal_y) do
      True
    end

    dummy = check(robot,cli_proc_name)
    if !(dummy) do
      robot=move(robot)
      dummy = check(robot,cli_proc_name)
      if solve(robot,visited,cli_proc_name,goal_x,goal_y) do
        True
      else
        robot = left(robot)
        dummy = check(robot,cli_proc_name)
    ...
    if !(check(robot,cli_proc_name)) do
      robot=move(robot)
      dummy = check(robot,cli_proc_name)
      if solve(robot,visited,cli_proc_name,goal_x,goal_y) do
        True
  ...
  def stop(robot, goal_x, goal_y, cli_proc_name) do
    visited = 0
    dummy = solve(robot,visited,cli_proc_name,goal_x,goal_y)
